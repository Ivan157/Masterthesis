...
stan_samples <- sampling(m, data = data_list, iter=100000, verbose=T, chain=4, control = list(adapt_delta = 0.90)) # optimalere Lösung
...
plot(stan_samples)
Stan_summary <- summary(stan_samples, pars = c("d[2]","d_II[2]", "T[1]", "T[2]", "s_d", "totalresdev", "dev", "theta"), probs = c(0.025, 0.975))$summary 
Stan_summary
