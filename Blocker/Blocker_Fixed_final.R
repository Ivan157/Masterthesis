...
# Compiling
m <- stan_model('Model_Fixed_final.stan')
m <- stan_model('Model_Fixed_final.stan')

# Simulation 
stan_samples <- sampling(m, data = data_list, iter=40000, verbose=T,  init ="all.inits", chain=4) 
...
plot(stan_samples)
Stan_summary <- summary(stan_samples, pars = c("d[2]", "d_II[2]", "T[1]", "T[2]", "totalresdev"), probs = c(0.025, 0.975))$summary 
Stan_summary
...
library(car)
scatterplot(c(fertige_Daten_w_ik[,"w_ik_neg"], fertige_Daten_w_ik[,"w_ik"]), c(leverage_I, leverage_II),  main="Leverage plot for the fixed effects model", xlim=c(-3,3), ylim=c(0,4.5), xlab=expression('w'[ik]), ylab=expression('leverage'[ik]), regLine =F, smooth=F, boxplots=F )                
