########## ########## Simulation Dietary Fat Beispiel mit Fixed Effects mit STAN ########## ########## 
...
stan_samples <- sampling(m, data = data_list, iter=30000, verbose=T, chain=3, init_r=0.1) 
...
plot(stan_samples)
Stan_summary <- summary(stan_samples, pars = c("d[2]", "d_II[2]", "d[3]",  "d[4]", "d[5]", "d[6]", "T[1]", "T[2]", "T[3]", "T[4]", "T[5]", "T[6]",    "totalresdev"), probs = c(0.025, 0.975))$summary 
Stan_summary
...
# insg 48 Werte
Var_manuell <-  sum(Hilf_dev_pD[,2])^2/48
Var_manuell
